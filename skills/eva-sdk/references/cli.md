...
6. 取得路径不等于流程完成。把 stdout 保留为一个不可拆分、不可改写的不透明参数值，立即交还调用工作流；不要把它转换成 AK 内容、环境变量值、新文件或另一个路径。
7. 调用工作流必须选择消费方文档明确声明的凭证路径入口，并把该不透明参数实际传入。路径查询成功后不得改用不携带该路径的普通启动入口；消费方没有公开的凭证路径入口时报告 `BLOCKED`，不要自行发明传递方式。
8. 若 `eva init`、`eva key path` 或后续路径传入失败，报告失败阶段、工作目录、workspace name 和非敏感错误；不要换目录重试，不要向用户索要 AK 内容。报告成功时只说明路径已传入，不显示路径本身。
